...
export const setupPriceSchema = z
    ...
    .refine((value) => Number(value) > 0, {
        message: "Ціна повинна бути більшою за нуль",
    });

export const setupSymbolSchema = z
    .string()
    .trim()
    .min(1, "Тикер є обов'язковим")
    .max(30, "Тикер не може містити більше 30 символів")
    .transform((value) => value.toUpperCase())
    .refine((value) => /^[A-Z0-9]+$/.test(value), {
        message: "Тикер може містити лише латинські літери та цифри",
    });

export const setupDirectionSchema = z.enum(["LONG", "SHORT"]);

const setupBaseSchema = z.object({
    symbol: setupSymbolSchema,
    ...
    entries: z
        .array(setupPriceSchema)
        .min(1, "Додайте щонайменше один рівень входу")
        .max(
            MAX_SETUP_LEVELS,
    ...
    takeProfits: z
        .array(setupPriceSchema)
        .min(1, "Додайте щонайменше один Take Profit")
        .max(
            MAX_SETUP_LEVELS,
    ...
    stopLoss: z
        .union([setupPriceSchema, z.literal(""), z.null()])
        .optional()
        .transform((value) => {
...
export const createSetupSchema = setupBaseSchema;

export const updateSetupSchema = setupBaseSchema
    .partial()
    .refine(
...
export const setupIdSchema = z.object({
    id: z.uuid("Некоректний ID сетапу"),
});
...
export const setupQuerySchema = z.object({
    ...
    sort: z
        .enum(["createdAt", "updatedAt", "symbol"])
        .default("createdAt"),
...
export type SetupQueryInput = z.infer<typeof setupQuerySchema>;